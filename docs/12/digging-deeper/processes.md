...
Within this closure, you may define the processes that belong to the pool. Once a process pool is started via the `start` method, you may access the [collection](/docs/{{version}}/digging-deeper/collections) of running processes via the `running` method:
