...
In Laravel, exception reporting is used to log exceptions or send them to an external service like [Sentry](https://github.com/getsentry/sentry-laravel) or [Flare](https://flareapp.io). By default, exceptions will be logged based on your [logging](/docs/{{version}}/basics/logging) configuration. However, you are free to log exceptions however you wish.
...
> [!NOTE]
> To customize the exception reporting for a given exception, you may also utilize [reportable exceptions](/docs/{{version}}/basics/errors#renderable-exceptions).
...
When messages are written to your application's [logs](/docs/{{version}}/basics/logging), the messages are written at a specified [log level](/docs/{{version}}/basics/logging#log-levels), which indicates the severity or importance of the message being logged.
...
> [!NOTE]
> You may type-hint any required dependencies of the `report` method and they will automatically be injected into the method by Laravel's [service container](/docs/{{version}}/architecture-concepts/container).
